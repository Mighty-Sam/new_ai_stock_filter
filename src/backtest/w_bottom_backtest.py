...
from src.screener.w_bottom import evaluate_w_bottom

logger = logging.getLogger(__name__)

STRATEGY_LABEL = "N漲W底假跌破（次高點停利／收盤跌破停損）"
HISTORY_YEARS = 3
SUMMARY_CACHE_PATH = Path("data/w_bottom_backtest_summary.json")
TRADES_CSV_PATH = Path("data/w_bottom_backtest_trades.csv")
CACHE_TTL_HOURS = 24
MIN_WARMUP = 130
MIN_FORWARD = 60
...
def load_cached_summary() -> Optional[BacktestSummary]:
    ...
    try:
        ...
        logger.warning("讀取 N漲W底假跌破回測快取失敗: %s", exc)
        return None
...
def _backtest_single_stock(
    ...
    for i in range(MIN_WARMUP, len(df) - MIN_FORWARD):
        ...
        subset = df.iloc[: i + 1]
        result = evaluate_w_bottom(subset, stock_code=stock_code)
        if result is None:
            continue
...
def run_w_bottom_backtest(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="N漲W底假跌破回測"):
            try:
    ...
    logger.info(
        "N漲W底假跌破回測完成：%d 檔有資料 / %d 檔，%d 信號，%d 筆交易",
        stocks_with_data,
        len(codes),
        signal_count,
        len(all_trades),
...
def get_or_run_w_bottom_backtest(
    refresh: bool = False,
    max_workers: int = 8,
    stock_limit: Optional[int] = None,
) -> BacktestSummary:
    if not refresh and _cache_fresh(SUMMARY_CACHE_PATH):
        ...
        if cached is not None:
            logger.info("使用 N漲W底假跌破回測快取")
            return cached

    return run_w_bottom_backtest(max_workers=max_workers, stock_limit=stock_limit)
