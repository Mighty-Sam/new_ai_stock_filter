...
from src.screener.limit_up_contraction import evaluate_limit_up_contraction

logger = logging.getLogger(__name__)

STRATEGY_LABEL = "漲停量縮整理（進場+20% 停利／收盤跌破 day1 低停損）"
HISTORY_YEARS = 3
SUMMARY_CACHE_PATH = Path("data/limit_up_contraction_backtest_summary.json")
TRADES_CSV_PATH = Path("data/limit_up_contraction_backtest_trades.csv")
CACHE_TTL_HOURS = 24
MIN_WARMUP = 30
MIN_FORWARD = 25
...
def load_cached_summary() -> Optional[BacktestSummary]:
    ...
    try:
        ...
        logger.warning("讀取漲停量縮整理回測快取失敗: %s", exc)
        return None
...
def _backtest_single_stock(
    ...
    for i in range(MIN_WARMUP, len(df) - MIN_FORWARD):
        ...
        subset = df.iloc[: i + 1]
        result = evaluate_limit_up_contraction(subset, stock_code=stock_code)
        if result is None:
            continue
...
def run_limit_up_contraction_backtest(
    max_workers: int = 8,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="漲停量縮整理回測"):
            try:
    ...
    logger.info(
        "漲停量縮整理回測完成：%d 檔有資料 / %d 檔，%d 信號，%d 筆交易",
        stocks_with_data,
        len(codes),
        signal_count,
        len(all_trades),
...
def get_or_run_limit_up_contraction_backtest(
    refresh: bool = False,
    max_workers: int = 8,
    stock_limit: Optional[int] = None,
) -> BacktestSummary:
    if not refresh and _cache_fresh(SUMMARY_CACHE_PATH):
        ...
        if cached is not None:
            logger.info("使用漲停量縮整理回測快取")
            return cached

    return run_limit_up_contraction_backtest(max_workers=max_workers, stock_limit=stock_limit)
