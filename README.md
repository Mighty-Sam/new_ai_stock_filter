...
# 強制重跑 3 年歷史回測
.venv/bin/python3.11 main.py --refresh-backtest
...
- 執行腳本：`scripts/daily_scan.sh`（週日 `--refresh-backtest`，其餘 `--skip-backtest`）
...
**輸出檔案：**
- `data/backtest.db` — 前瞻追蹤 SQLite
- `data/backtest_summary.json` — 歷史回測摘要快取
- `data/backtest_trades.csv` — 歷史回測交易明細

Telegram 摘要會附上歷史勝率與前瞻追蹤統計，並顯示每檔 **產業**（FinMind 產業別）與 **族群**（FinMind 產業鏈 sub_industry），以及今日信號的產業/族群分布，方便觀察族群輪動。產業 metadata 快取於 `data/stock_metadata.json`（7 天 TTL，首次或過期時向 FinMind 抓取）。
